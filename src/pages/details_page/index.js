...
class DetailsPage extends React.Component {
    ...
    componentDidMount() {
        ...
        let issue_number = parseInt(this.props.match.params.issueNumber);
        ...
        db.collection('missing_persons').where('issueNumber', '==', issue_number).get()
            .then(snapshot => {
                if (snapshot.empty) {
                ...
                this.setState({
                    isLoading: false,
                    personData: snapshot.docs[0].data()
                })
            })
            .catch(err => {
